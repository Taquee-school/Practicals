...
async function initiateElements(parent: HTMLDivElement) {
  inputs.disByCsr = parent.querySelector<HTMLInputElement>("#scg-distance-by-csr-input");
  inputs.revByCsr = parent.querySelector<HTMLInputElement>("#scg-revolution-by-csr-input");
  inputs.pitch = parent.querySelector<HTMLInputElement>("#scg-pitch-input");
  inputs.divOnCsr = parent.querySelector<HTMLInputElement>("#scg-divisions-of-csr-input");
  inputs.leastCount = parent.querySelector<HTMLInputElement>("#scg-least-count-input");
  inputs.zeroError1 = parent.querySelector<HTMLInputElement>("#scg-zero-error-1-input");
  inputs.zeroError2 = parent.querySelector<HTMLInputElement>("#scg-zero-error-2-input");
  inputs.zeroError3 = parent.querySelector<HTMLInputElement>("#scg-zero-error-3-input");
  inputs.meanZeroError = parent.querySelector<HTMLInputElement>("#scg-mean-zero-error-input");
  inputs.zeroCorrection = parent.querySelector<HTMLInputElement>("#scg-zero-correction-input");
  inputs.density = parent.querySelector<HTMLInputElement>("#scg-density-input");
  inputs.meanDiameter = parent.querySelector<HTMLInputElement>("#scg-mean-diameter-input");
  inputs.correctedDiameter = parent.querySelector<HTMLInputElement>("#scg-corrected-diameter-input");
  inputs.massPerUnitLength = parent.querySelector<HTMLInputElement>("#scg-mass-per-unit-length-input");
}

function measureEssentials() {

  // Distance moved by circular scale
  if (!inputs.disByCsr_input) return {
    ok: false,
    message: "missing ui",
  };
...
function measureZeroError() {
  ...
  // Mean Zero Error
  if (!inputs.meanZeroError) return {
    ok: false,
    message: "missing ui",
  };
  ...
  // Zero Correction
  if (!inputs.zeroCorrection) return {
    ok: false,
    message: "missing ui",
  };
...
function evaluate_diameter_table(parent: HTMLDivElement) {
  ...
  while (true) {
    ...
    // diameter
    const diameter_input = table.querySelector<HTMLInputElement>(`#scg-d-obs-${row}`);
...
function measureDiameter() {
  ...
  // corrected diameter
  if (!inputs.correctedDiameter) return {
    ok: false,
    message: "missing ui",
  };
  const correctedDiameter = experimentVariables.meanDiameter + experimentVariables.zeroCorrection;
...
function measureMassPerUnitLength() {
  ...
  if (!inputs.massPerUnitLength) return {
    ok: false,
    message: "missing ui",
  };
  const area = Math.PI * (experimentVariables.correctedDiameter/2*1000)**2;
  experimentVariables.massPerUnitLength = area * experimentVariables.density;
  inputs.massPerUnitLength.value = experimentVariables.massPerUnitLength.toFixed(10);

  return { ok: true }
}

export async function run(parent: HTMLDivElement, options = {
  zeroError: true,
  mass: true,
}) {
