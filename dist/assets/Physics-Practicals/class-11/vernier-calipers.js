...
vnc_diagramDiv.appendChild(createTextField("practical-section-header", "DIAGRAM:"));
let vnc_diagram_light = document.createElement("img");
vnc_diagram_light.className = "practical-file-diagram", "light";
vnc_diagram_light.src = physicsFilePath_class11 + "class-11/assets/diagrams/light/vernier calipers.jpg";
...
vnc_diagram_dark.src = physicsFilePath_class11 + "class-11/assets/diagrams/dark/vernier calipers.jpg";
...
vnc_objectiveDiv.appendChild(createTextField("practical-section-header", "OBJECT:"));
vnc_objectiveDiv.appendChild(createTextField("practical-section-content", "To find the volume of the material of a hollow cylinder using vernier calipers."));
let vnc_apparatusDiv = createDiv("practical-section");
experimentDiv.appendChild(vnc_apparatusDiv);
vnc_apparatusDiv.appendChild(createTextField("practical-section-header", "APPARATUS USED:"));
vnc_apparatusDiv.appendChild(createTextField("practical-section-content", "A hollow cylinder, vernier calipers."));
let vnc_formulaDiv = createDiv("practical-section");
experimentDiv.appendChild(vnc_formulaDiv);
vnc_formulaDiv.appendChild(createTextField("practical-section-header", "FORMULA USED:"));
vnc_formulaDiv.appendChild(createTextField("practical-main-formula", "V = π(de2he - di2hi)/4"));
vnc_formulaDiv.appendChild(createTextField("practical-section-content", "Where,"));
...
vnc_observationDiv.appendChild(createTextField("practical-section-header", "OBSERVATION:"));
let vnc_mainScaleDivInput = createInput("vnc-distance-moved-by-circular-scale-input", "number", 0, measureEssentials_vnc);
vnc_observationDiv.appendChild(createPAS("1", createInputDiv("Value of one smallest division of the main scale (x): ", vnc_mainScaleDivInput, "cm"), true));
let vnc_vernierScaleDivInput = createInput("vnc-revolution-of-circular-scale-input", "number", 0, measureEssentials_vnc);
vnc_observationDiv.appendChild(createPAS("2", createInputDiv("Number of Total division on vernier scale (y): ", vnc_vernierScaleDivInput), true));
let vnc_leastCountInput = createInput("vnc-least-count-input", "number", 0, null, true);
vnc_observationDiv.appendChild(createPAS("3", createInputDiv("Least Count: ", vnc_leastCountInput, "mm"), true));
...
let vnc_ze1Input = createInput("vnc-zero-error-1-input", "number", 0, measureEssentials_vnc);
vnc_zeroErrorDiv.appendChild(createPAS("a", createInputDiv("Zero Error: ", vnc_ze1Input, "mm"), true));
let vnc_ze2Input = createInput("vnc-zero-error-2-input", "number", 0, measureEssentials_vnc);
vnc_zeroErrorDiv.appendChild(createPAS("b", createInputDiv("Zero Error: ", vnc_ze2Input, "mm"), true));
let vnc_ze3Input = createInput("vnc-zero-error-3-input", "number", 0, measureEssentials_vnc);
vnc_zeroErrorDiv.appendChild(createPAS("c", createInputDiv("Zero Error: ", vnc_ze3Input, "mm"), true));
let vnc_meanZeroErrorInput = createInput("vnc-mean-zero-error-input", "number", 0, null, true);
vnc_observationDiv.appendChild(createPAS("7", createInputDiv("Mean zero error: ", vnc_meanZeroErrorInput, "mm"), true));
let vnc_zeroCorrectionInput = createInput("vnc-zero-correction-input", "number", 0, null, true);
vnc_observationDiv.appendChild(createPAS("8", createInputDiv("Zero correction: ", vnc_zeroCorrectionInput, "mm"), true));
...
vnc_addExDRow();
vnc_externalDiameterTableDiv.appendChild(createButton("vnc-ex-d-add-row-btn", "add-row-btn toggle-effect ", createIcon("bold", "plus"), "Add Row", vnc_addExDRow));
let vnc_meanExternalDiameterInput = createInput("vnc-mean-observed-diameter-input", "number", 0);
vnc_observationDiv.appendChild(createInputDiv("Mean observed external diameter: ", vnc_meanExternalDiameterInput, "cm"));
let vnc_correctedExternalDiameterInput = createInput("vnc-corrected-diameter-input", "number", 0);
vnc_observationDiv.appendChild(createInputDiv("Corrected external diameter: ", vnc_correctedExternalDiameterInput, "cm"));
...
vnc_addInDRow();
vnc_internalDiameterTableDiv.appendChild(createButton("vnc-in-d-add-row-btn", "add-row-btn toggle-effect ", createIcon("bold", "plus"), "Add Row", vnc_addInDRow));
let vnc_meanInternalDiameterInput = createInput("vnc-mean-observed-internal-diameter-input", "number", 0);
vnc_observationDiv.appendChild(createInputDiv("Mean observed internal diameter: ", vnc_meanInternalDiameterInput, "cm"));
let vnc_correctedInternalDiameterInput = createInput("vnc-corrected-internal-diameter-input", "number", 0);
vnc_observationDiv.appendChild(createInputDiv("Corrected internal diameter: ", vnc_correctedInternalDiameterInput, "cm"));
...
vnc_addExHRow();
vnc_externalHeightTableDiv.appendChild(createButton("vnc-ex-h-add-row-btn", "add-row-btn toggle-effect ", createIcon("bold", "plus"), "Add Row", vnc_addExHRow));
let vnc_meanExternalHeightInput = createInput("vnc-mean-observed-external-height-input", "number", 0);
vnc_observationDiv.appendChild(createInputDiv("Mean observed external height: ", vnc_meanExternalHeightInput, "cm"));
let vnc_correctedExternalHeightInput = createInput("vnc-corrected-external-height-input", "number", 0);
vnc_observationDiv.appendChild(createInputDiv("Corrected external height: ", vnc_correctedExternalHeightInput, "cm"));
...
vnc_addInHRow();
vnc_internalHeightTableDiv.appendChild(createButton("vnc-in-h-add-row-btn", "add-row-btn toggle-effect ", createIcon("bold", "plus"), "Add Row", vnc_addInHRow));
let vnc_meanInternalHeightInput = createInput("vnc-mean-observed-internal-height-input", "number", 0);
vnc_observationDiv.appendChild(createInputDiv("Mean observed internal height: ", vnc_meanInternalHeightInput, "cm"));
let vnc_correctedInternalHeightInput = createInput("vnc-corrected-internal-height-input", "number", 0);
vnc_observationDiv.appendChild(createInputDiv("corrected internal height: ", vnc_correctedInternalHeightInput, "cm"));
let vnc_resultDiv = createDiv("practical-section");
experimentDiv.appendChild(vnc_resultDiv);
vnc_resultDiv.appendChild(createTextField("practical-section-header", "RESULT:"));
let vnc_masspulInput = createInput("vnc-mass-per-unit-length-input", "number", 0);
vnc_masspulInput.style.width = "100px";
vnc_resultDiv.appendChild(createInputDiv("The mass per unit length of the wire is found to be: ", vnc_masspulInput, "kg/m"));
let vnc_precautionsDiv = createDiv("practical-section");
experimentDiv.appendChild(vnc_precautionsDiv);
vnc_precautionsDiv.appendChild(createTextField("practical-section-header", "PRECAUTIONS AND SOURCES OF ERRORS:"));
...
//# sourceMappingURL=vernier-calipers.js.map