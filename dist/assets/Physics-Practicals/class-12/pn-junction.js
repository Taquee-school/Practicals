...
function measureResistance_pnj() {
    ...
    for (let i = 1; i <= pnj_rowCount; i++) {
        let a_div = parseFloat(document.getElementById(`pnj-table-a-div-${i}`).value) || 0;
        let v_div = parseFloat(document.getElementById(`pnj-table-v-div-${i}`).value) || 0;
...
pnj_objectiveDiv.appendChild(createTextField("practical-section-header", "OBJECTIVE:"));
pnj_objectiveDiv.appendChild(createTextField("practical-section-content", "To draw the characteristics curve of a forward biased P-N junction diode and hence to determine the static and dynamic resistance of the same."));
// #endregion Objective
// #region Apparatus
let pnj_apparatusDiv = createDiv("practical-section");
experimentDiv.appendChild(pnj_apparatusDiv);
pnj_apparatusDiv.appendChild(createTextField("practical-section-header", "APPARATUS USED:"));
pnj_apparatusDiv.appendChild(createTextField("practical-section-content", "A P-N junction Diode, a battery eliminator (0-2V), rheostat, a milli-ammeter, a voltmeter, connecting wire etc."));
// #endregion Apparatus
// #region Formula
let pnj_formulaDiv = createDiv("practical-section");
experimentDiv.appendChild(pnj_formulaDiv);
pnj_formulaDiv.appendChild(createTextField("practical-section-header", "FORMULA USED:"));
pnj_formulaDiv.appendChild(createTextField("practical-section-content", "The static resistance of the junction diode is given by:"));
pnj_formulaDiv.appendChild(createTextField("practical-main-formula", "R(static) = V(av)/I(av)"));
pnj_formulaDiv.appendChild(createTextField("practical-section-content", "Here V(av) and I(av) are the average values of voltage and current respectively over a segment (starting from origin) of V-I plot."));
pnj_formulaDiv.appendChild(createTextField("practical-section-content", "Whereas, the dynamic resistance of the junction is given by:"));
pnj_formulaDiv.appendChild(createTextField("practical-main-formula", "R(dynamic) = ∆V/∆I"));
pnj_formulaDiv.appendChild(createTextField("practical-section-content", "Where, ∆V and ∆I are the changes in applied voltage and current, calculated by drawing a tangent on the V-I plot (to find its slope)."));
// #endregion Formula
// #region Observation
let pnj_observationDiv = createDiv("practical-section");
experimentDiv.appendChild(pnj_observationDiv);
pnj_observationDiv.appendChild(createTextField("practical-section-header", "OBSERVATION:"));
let pnj_voltmeterRangeInput = createInput("pnj-voltmeter-range-input", "number", 0, null);
pnj_observationDiv.appendChild(createInputDiv("Range of voltmeter: ", pnj_voltmeterRangeInput, "V"));
let pnj_voltmeterLcInput = createInput("pnj-voltmeter-least-count-input", "number", 0, measureEssentials_pnj);
pnj_observationDiv.appendChild(createInputDiv("Least count of voltmeter: ", pnj_voltmeterLcInput, "V"));
let pnj_ammeterRangeInput = createInput("pnj-ammeter-range-input", "number", 0, null);
pnj_observationDiv.appendChild(createInputDiv("Range of ammeter: ", pnj_ammeterRangeInput, "mA"));
let pnj_ammeterLcInput = createInput("pnj-ammeter-least-count-input", "number", 0, measureEssentials_pnj);
pnj_observationDiv.appendChild(createInputDiv("Least count of ammeter: ", pnj_ammeterLcInput, "mA"));
pnj_observationDiv.appendChild(createTextField("observation-table-header", "Table for the measurement of current"));
...
pnj_resultDiv.appendChild(createTextField("practical-section-header", "RESULT:"));
pnj_resultDiv.appendChild(createPAS("1", "The V-I characteristics of a p-n junction is not a straight line, therefore p-n junction diode is a non-ohmic resistance in nature."));
let pnj_staticResistanceInput = createInput("pnj-static-resistance-input", "number", 0, null, true);
let pnj_dynamicResistanceInput = createInput("pnj-dynamic-resistance-input", "number", 0, null, true);
...
pnj_precautionsDiv.appendChild(createTextField("practical-section-header", "PRECAUTIONS:"));
pnj_precautionsDiv.appendChild(createPAS("1", "All connections should be tight and neat."));
pnj_precautionsDiv.appendChild(createPAS("2", "Voltmeter and milli-ammeter of appropriate least counts and ranges should be selected."));
pnj_precautionsDiv.appendChild(createPAS("3", "The pointer of meters should either be adjusted to read zero, when no current is passing through them, or zero error of the instrument should be taken into account."));
pnj_precautionsDiv.appendChild(createPAS("4", "The battery connections of p-n junction diode should be checked and in forward biasing, it should be ensured that p is connected to positive and n to the negative of the battery."));
pnj_precautionsDiv.appendChild(createPAS("5", "Never cross the limits specified by the manufacturer otherwise the diode will get damaged."));
...
pnj_soeDiv.appendChild(createTextField("practical-section-header", "SOURCES OF ERRORS:"));
pnj_soeDiv.appendChild(createPAS("1", "The diode is not suitably biased."));
pnj_soeDiv.appendChild(createPAS("2", "Zero error is not correctly estimated."));
pnj_soeDiv.appendChild(createPAS("3", "Current is fixed through the diode for long duration."));
// #endregion Sources of Errors
//# sourceMappingURL=pn-junction.js.map