...
function measureResistance_ohl() {
  ...
  for (let i = 1; i <= ohl_rowCount; i++) {
    ...
    if (
      document.getElementById(`ohl-a-div-${i}`).classList.contains("invalid")
    ) {
      document.getElementById(`ohl-a-div-${i}`).classList.remove("invalid");
    }
    if (
      document.getElementById(`ohl-v-div-${i}`).classList.contains("invalid")
    ) {
...
ohl_diagramDiv.appendChild(
  createImg(
    ...
    "light",
    base + "/diagrams/class-12/light/ohms law.jpg",
  ),
);
ohl_diagramDiv.appendChild(
  createImg(
    "practical-file-diagram",
    "dark",
    base + "/diagrams/class-12/dark/ohms law.jpg",
  ),
);
// #endregion Diagram
...
ohl_addRow();
ohl_resistanceTableDiv.appendChild(
  toggleEffect(
    createButton(
      "ohl-add-row-btn",
