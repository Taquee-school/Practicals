...
async function initiateElements(parent) {
    inputs.disByCsr = parent.querySelector("#scg-distance-by-csr-input");
    inputs.revByCsr = parent.querySelector("#scg-revolution-by-csr-input");
    inputs.pitch = parent.querySelector("#scg-pitch-input");
    inputs.divOnCsr = parent.querySelector("#scg-divisions-of-csr-input");
    inputs.leastCount = parent.querySelector("#scg-least-count-input");
    inputs.zeroError1 = parent.querySelector("#scg-zero-error-1-input");
    inputs.zeroError2 = parent.querySelector("#scg-zero-error-2-input");
    inputs.zeroError3 = parent.querySelector("#scg-zero-error-3-input");
    inputs.meanZeroError = parent.querySelector("#scg-mean-zero-error-input");
    inputs.zeroCorrection = parent.querySelector("#scg-zero-correction-input");
    inputs.density = parent.querySelector("#scg-density-input");
    inputs.meanDiameter = parent.querySelector("#scg-mean-diameter-input");
    inputs.correctedDiameter = parent.querySelector("#scg-corrected-diameter-input");
    inputs.massPerUnitLength = parent.querySelector("#scg-mass-per-unit-length-input");
}
...
function evaluate_diameter_table(parent) {
    const table = parent.querySelector("#scg-diameter-table");
    if (!table)
        return {
            ok: false,
            message: "missing ui",
    ...
    while (true) {
        // least count
        const lc_input = table.querySelector(`#scg-lc-${row}`);
        if (!lc_input)
            break;
        lc_input.value = experimentVariables.leastCount.toFixed(3);
        // main scale reading
        const msr_input = table.querySelector(`#scg-msr-${row}`);
        if (!msr_input)
            break;
        const msr = parseFloat(msr_input.value) || 0;
        // circular scale reading
        const csr_input = table.querySelector(`#scg-csr-${row}`);
        if (!csr_input)
            break;
        const csr = parseFloat(csr_input.value) || 0;
        // diameter
        const diameter_input = table.querySelector(`#scg-d-obs-${row}`);
        if (!diameter_input)
            break;
...
export async function run(parent, options = {
    zeroError: true,
    mass: true,
}) {
