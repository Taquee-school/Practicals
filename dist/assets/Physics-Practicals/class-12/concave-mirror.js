...
function ccm_addRow() {
    ccm_rowCount++;
    setTimeout(() => {
        let inp = createInput(`ccm-s-no-${ccm_rowCount}`, "number", ccm_rowCount, null, true);
        ccm_sNoColumn.appendChild(inp);
        inp.style.animation = "appear 0.5s ease";
    }, 10);
    setTimeout(() => {
        let inp = createInput(`ccm-table-P-${ccm_rowCount}`, "number", 0, measureFocalLength_ccm);
        ccm_PColumn.appendChild(inp);
        inp.style.animation = "appear 0.5s ease";
    }, 50);
    setTimeout(() => {
        let inp = createInput(`ccm-table-O-${ccm_rowCount}`, "number", 0, measureFocalLength_ccm);
        ccm_OColumn.appendChild(inp);
        inp.style.animation = "appear 0.5s ease";
    }, 100);
    setTimeout(() => {
        let inp = createInput(`ccm-table-I-${ccm_rowCount}`, "number", 0, measureFocalLength_ccm);
        ccm_IColumn.appendChild(inp);
        inp.style.animation = "appear 0.5s ease";
    }, 150);
    setTimeout(() => {
        let inp = createInput(`ccm-table-U-obs-${ccm_rowCount}`, "number", 0, null, true);
        ccm_UObsColumn.appendChild(inp);
        inp.style.animation = "appear 0.5s ease";
    }, 200);
    setTimeout(() => {
        let inp = createInput(`ccm-table-U-cor-${ccm_rowCount}`, "number", 0, null, true);
        ccm_UCorColumn.appendChild(inp);
        inp.style.animation = "appear 0.5s ease";
    }, 250);
    setTimeout(() => {
        let inp = createInput(`ccm-table-I-obs-${ccm_rowCount}`, "number", 0, null, true);
        ccm_IObsColumn.appendChild(inp);
        inp.style.animation = "appear 0.5s ease";
    }, 300);
    setTimeout(() => {
        let inp = createInput(`ccm-table-I-cor-${ccm_rowCount}`, "number", 0, null, true);
        ccm_ICorColumn.appendChild(inp);
        inp.style.animation = "appear 0.5s ease";
    }, 350);
    setTimeout(() => {
        let inp = createInput(`ccm-table-F-${ccm_rowCount}`, "number", 0, null, true);
...
ccm_objectiveDiv.appendChild(createTextField("practical-section-header", "OBJECT:"));
ccm_objectiveDiv.appendChild(createTextField("practical-section-content", "To find the value of 'v' for different values of 'u' in case of a concave mirror and hence, to find the focal length 'f' of the mirrors by plotting graph between 'u' & 'v'."));
// #endregion Objective
// #region Apparatus
let ccm_apparatusDiv = createDiv("practical-section");
experimentDiv.appendChild(ccm_apparatusDiv);
ccm_apparatusDiv.appendChild(createTextField("practical-section-header", "APPARATUS USED:"));
ccm_apparatusDiv.appendChild(createTextField("practical-section-content", "An optical bench along with three uprights, one mirror holder, two needles, concave mirror, a knitting needle and a meter scale."));
// #endregion Apparatus
// #region Formula
let ccm_formulaDiv = createDiv("practical-section");
experimentDiv.appendChild(ccm_formulaDiv);
ccm_formulaDiv.appendChild(createTextField("practical-section-header", "FORMULA USED:"));
ccm_formulaDiv.appendChild(createTextField("practical-section-content", "The relation between the object distance 'u' and the image distance 'v' and the focal length 'f' of a concave mirror is given by"));
ccm_formulaDiv.appendChild(createTextField("practical-main-formula", "1/f = 1/v + 1/u"));
ccm_formulaDiv.appendChild(createTextField("practical-section-content", "Or"));
ccm_formulaDiv.appendChild(createTextField("practical-main-formula", "f = uv/(u+v)"));
ccm_formulaDiv.appendChild(createTextField("practical-section-content", "In the case of concave mirrors, for real images both 'u' & 'v' are negative as per new Cartesian sign convention. The focal length 'f' is also negative."));
// #endregion Formula
// #region Observation
let ccm_observationDiv = createDiv("practical-section");
experimentDiv.appendChild(ccm_observationDiv);
ccm_observationDiv.appendChild(createTextField("practical-section-header", "OBSERVATION:"));
let ccm_lengthOfNeedleInput = createInput("ccm-length-of-knitting-needle-input", "number", 0, measureEssentials_ccm);
ccm_observationDiv.appendChild(createPAS("1", createInputDiv("Length of knitting needle (x): ", ccm_lengthOfNeedleInput, "cm."), true));
let ccm_distanceMirrorObjectInput = createInput("ccm-distance-between-mirror-and-object-input", "number", 0, measureEssentials_ccm);
ccm_observationDiv.appendChild(createPAS("2", createInputDiv("Observed distance between the mirror and the object needle 'O' when the Knitting needle is introduced between them (y): ", ccm_distanceMirrorObjectInput, "cm."), true));
let ccm_distanceMirrorImageInput = createInput("ccm-distance-between-mirror-and-image-input", "number", 0, measureEssentials_ccm);
ccm_observationDiv.appendChild(createPAS("3", createInputDiv("Observed distance between the mirror and the iamge needle 'I' when the Knitting needle is introduced between them (z): ", ccm_distanceMirrorImageInput, "cm."), true));
let ccm_indexErrorUInput = createInput("ccm-index-error-u-input", "number", 0, null, true);
ccm_observationDiv.appendChild(createPAS("4", createInputDiv("Index error for 'u' = e1 = (y-x):", ccm_indexErrorUInput, "cm."), true));
let ccm_indexCorrectionUInput = createInput("ccm-index-correction-u-input", "number", 0, null, true);
ccm_observationDiv.appendChild(createPAS("5", createInputDiv("Index correction for 'u' = (-e1):", ccm_indexCorrectionUInput, "cm."), true));
let ccm_indexErrorVInput = createInput("ccm-index-error-v-input", "number", 0, null, true);
ccm_observationDiv.appendChild(createPAS("6", createInputDiv("Index error for 'v' = e2 = (z-x):", ccm_indexErrorVInput, "cm."), true));
let ccm_indexCorrectionVInput = createInput("ccm-index-correction-v-input", "number", 0, null, true);
ccm_observationDiv.appendChild(createPAS("7", createInputDiv("Index correction for 'v' = (-e2):", ccm_indexCorrectionVInput, "cm."), true));
ccm_observationDiv.appendChild(createTextField("observation-table-header", "Table for the measurement of focal length of mirror"));
...
ccm_resultDiv.appendChild(createTextField("practical-section-header", "RESULT:"));
let ccm_focalLengthCalculatedInput = createInput("ccm-mean-focal-length-calculated-input", "number", 0);
ccm_resultDiv.appendChild(createPAS("1", createInputDiv("Mean value of focal length by computations found to be: ", ccm_focalLengthCalculatedInput, "cm."), true));
let ccm_focalLengthGraphInput = createInput("ccm-mean-focal-length-by-graph-input", "number", 0);
ccm_resultDiv.appendChild(createPAS("2", createInputDiv("The focal length of the given cocave mirror as determined by plotting graph(Hyperbola) between 'u' and 'v' is found to be: ", ccm_focalLengthGraphInput, "cm."), true));
// #endregion Result
// #region Precautions
let ccm_precautionsDiv = createDiv("practical-section");
experimentDiv.appendChild(ccm_precautionsDiv);
ccm_precautionsDiv.appendChild(createTextField("practical-section-header", "PRECAUTIONS:"));
...
ccm_SOEDiv.appendChild(createTextField("practical-section-header", "SOURCES OF ERRORS:"));
...
//# sourceMappingURL=concave-mirror.js.map